...
tagTree = mark 0
	where
		mark :: Int -> Tree a -> Tree (a, Int)
		mark _ Leaf = Leaf
		mark offset (Node v lt rt) = Node (v, offset + n) (mark offset lt) $ mark (offset + n + 1) rt
			where
				n = nodes lt
...
while cond body = helper cond body []
	where
		helper cond body res = do
			value <- cond
			case value of
				True -> do
					newRes <- body
					helper cond body (newRes:res)
				False -> return $ reverse res
