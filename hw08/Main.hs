...
goodChar :: Char -> Parser Char
goodChar ch = satisfy (== ch)

sat :: String -> Parser String
sat [] = pure []
sat (x : xs) = (:) <$> goodChar x <*> (sat xs <|> pure [])
...
maybeP p = Just <$> (just *> char ' ' *> p) <|> nothing *> pure Nothing

-- (0.5 балла)
listP :: Parser a -> Parser [a]
listP p = brackets $ sepBy p comma
...
-- (0.5 балла)
listP' :: Parser a -> Parser [a]
listP' p = brackets $ sepBy p $ spaced comma
